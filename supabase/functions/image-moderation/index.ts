...
/// Checks a selected image before it is uploaded as either a review photo or
/// a profile photo. The Cloud Vision API key stays in Supabase secrets.
Deno.serve(async (request) => {
  ...
  if (!apiKey) {
    return response({ message: 'Image moderation is not configured.' }, 503)
  }
  ...
  try {
    ...
    if (!supportedContentTypes.has(contentType)) {
      return response({ approved: false, message: 'Only JPEG, PNG, or WebP images are allowed.' }, 400)
    }
    if (image.length === 0 || image.length > maximumBase64Length) {
      return response({ approved: false, message: 'The selected image is invalid or too large.' }, 400)
    }

    const safeSearch = await detectSafeSearch(image, apiKey)
    if (isUnsafe(safeSearch)) {
      return response({ approved: false, message: 'This image cannot be used because it contains inappropriate content.' })
    }
    return response({ approved: true })
  } catch (error) {
    console.error('Image moderation failed', error)
    return response({ message: 'Unable to verify this image. Please try again later.' }, 503)
  }
})
